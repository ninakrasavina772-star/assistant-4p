...
import type { CosmeticsAutoDetectResult } from "@/lib/podruzhkaCosmeticsAutoMapping";
import {
...
type Props = {
  detection: CosmeticsAutoDetectResult;
  rowCount: number;
};

export function PodruzhkaCosmeticsDetectedLayout({ detection, rowCount }: Props) {
  return (
    <div className="space-y-3">
      <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
        <p className="font-semibold">Файл распознан — как в образце Подружка (косметика)</p>
      ...
      <table className="w-full text-sm border border-slate-200 rounded-lg overflow-hidden">
        ...
        <tbody>
          ...
          {detection.textColumns.map((c) => (
            <tr key={`text-${c.col}`} className="border-t border-slate-100 bg-violet-50/50">
              <td className="px-3 py-2 text-violet-900">
                {c.role}
                {c.header === "model" || c.header.startsWith("benefit") ? " — тексты карточки" : ""}
              </td>
      ...
      {detection.missing.length > 0 ? (
        <p className="text-sm text-red-700">
          Не найдены колонки:{" "}
          {detection.missing
            .map((k) => PODRUZHKA_COSMETICS_FIELD_LABELS[k as PodruzhkaCosmeticsFieldKey])
            .join(", ")}
