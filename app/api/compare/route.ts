...
import { fetchAllProductsInRubric, fetchMergedRubricsProductIds, fetchMergedRubricsProducts, fetchNoveltyIdsSlicePage, fetchProductsByIds, applyRubricFetchPipeline, type RubricFetchPipeline } from "@/lib/fourpartners";
...
import type { AttrMatchOptions, CompareBrandFilterInfo, CompareExcludeIdsAInfo, CompareModelFilterInfo, CompareResult, FpProduct, NameLocale, NoveltiesFullExportResult, NoveltyIdsNoEanOnAResult, NoveltyIdsSliceResult, NoveltyIdsStageResult, SingleSiteDupsResult, UnlikelySearchInfo } from "@/lib/types";
...
function resolveToken(bodyToken: unknown, leg: "A" | "B"): string {
  ...
  if (leg === "A") return (envA.length >= MIN_TOKEN_LEN ? envA : "") || (envShared.length >= MIN_TOKEN_LEN ? envShared : "");
...
export function parseRubricBIds(body: Record<string, unknown>): number[] {
...
export function parseNoveltyIdsFromBody(raw: unknown): number[] {
  if (!Array.isArray(raw)) return [];
  const seen = new Set<number>();
  const out: number[] = [];
  for (const x of raw) {
...
function parseAttrMatch(raw: unknown): AttrMatchOptions | undefined {
  ...
  const volume = Boolean(o.volume), shade = Boolean(o.shade), color = Boolean(o.color);
...
function buildUnlikelySearch(attr?: AttrMatchOptions): UnlikelySearchInfo {
  return { attempted: true, volume: Boolean(attr?.volume), shade: Boolean(attr?.shade), color: Boolean(attr?.color) };
}

function buildBrandFilterInfo(brands: string[], brandMatch: BrandMatchMode, excludedMissingA: number, excludedNotInListA: number, excludedMissingB: number, excludedNotInListB: number): CompareBrandFilterInfo | undefined {
  if (!brands.length) return undefined;
  return { enabled: true, matchMode: brandMatch, brandsSample: brands.slice(0, 50), totalBrands: brands.length, excludedMissingBrandA: excludedMissingA, excludedMissingBrandB: excludedMissingB, excludedNotInListA: excludedNotInListA, excludedNotInListB: excludedNotInListB };
}

function buildModelFilterInfo(models: string[], modelMatch: ModelMatchMode, excludedNotInListA: number, excludedNotInListB: number): CompareModelFilterInfo | undefined {
  if (!models.length) return undefined;
  return { enabled: true, matchMode: modelMatch, modelsSample: models.slice(0, 50), totalModels: models.length, excludedNotInListA, excludedNotInListB };
}

function buildExcludeIdsInfo(excludeRaw: number[], excludeMeta?: { removedFromA: number; listIdsNotFoundInRubric: number }): CompareExcludeIdsAInfo | undefined {
  if (!excludeRaw.length) return undefined;
  return { enabled: true, listSize: excludeRaw.length, removedFromA: excludeMeta?.removedFromA ?? 0, listIdsNotFoundInRubric: excludeMeta?.listIdsNotFoundInRubric ?? 0 };
}

function applyBrandModelOnly(products: FpProduct[], pipe: Pick<RubricFetchPipeline, "brands" | "brandMatch" | "models" | "modelMatch">): FpProduct[] {
  let working = products;
  if (pipe.brands?.length) working = filterFpProductsByBrands(working, pipe.brands, pipe.brandMatch ?? "exact").products;
  if (pipe.models?.length) working = filterFpProductsByModels(working, pipe.models, pipe.modelMatch ?? "exact").products;
  return working;
}
...
async function resolveFeedCsvInput(
  label: string,
  urlRaw: unknown,
  textRaw: unknown
): Promise<string> {
...
export async function POST(req: NextRequest) {
  ...
  const siteVariation = typeof body.siteVariation === "string" && body.siteVariation.trim() ? body.siteVariation.trim() : "default";
  ...
  const excludeSet = excludeIdsRaw.length > 0 ? new Set<number>(excludeIdsRaw) : undefined;
  const pipeShared: RubricFetchPipeline = { brands: brandsRaw.length ? brandsRaw : undefined, brandMatch, models: modelsRaw.length ? modelsRaw : undefined, modelMatch };
  const pipeA: RubricFetchPipeline = { ...pipeShared, excludeIds: excludeSet, excludeIdsRaw: excludeIdsRaw.length ? excludeIdsRaw : undefined };
  const siteALabel = typeof body.siteALabel === "string" && body.siteALabel.trim() ? body.siteALabel.trim() : "A";
  const siteBLabel = typeof body.siteBLabel === "string" && body.siteBLabel.trim() ? body.siteBLabel.trim() : "B";

  try {
    if (body.comparePhase === "noveltyIdsSlice") {
      const tokenA = resolveToken(body.tokenA, "A"), tokenB = resolveToken(body.tokenB, "B");
      if (tokenA.length < MIN_TOKEN_LEN || tokenB.length < MIN_TOKEN_LEN) return NextResponse.json({ error: "Нужны ключи API для сайтов A и B (или переменные окружения)" }, { status: 400 });
      const sliceRaw = body.noveltyIdsSlice;
      if (!sliceRaw || typeof sliceRaw !== "object") return NextResponse.json({ error: "Укажите noveltyIdsSlice" }, { status: 400 });
      const sl = sliceRaw as Record<string, unknown>;
      const leg: "A" | "B" = sl.leg === "B" ? "B" : "A";
      const rubricId = Number(sl.rubricId), pageNum = Number(sl.page ?? 1);
      if (!Number.isFinite(rubricId) || rubricId < 1) return NextResponse.json({ error: "noveltyIdsSlice.rubricId" }, { status: 400 });
      const page = Number.isFinite(pageNum) && pageNum >= 1 ? Math.floor(pageNum) : 1;
      const token = leg === "A" ? tokenA : tokenB, pipe = leg === "A" ? pipeA : pipeShared;
      const slice = await fetchNoveltyIdsSlicePage(token, siteVariation, rubricId, page, pipe, leg);
      const payload: NoveltyIdsSliceResult = {
        resultKind: "noveltyIdsSlice", leg, rubricId, page: slice.page, ids: slice.ids, rawCatalogIdsBeforeExclude: slice.rawCatalogIdsBeforeExclude, hasMore: slice.hasMore, perPage: slice.perPage,
        statsSlice: { brandExcludedMissing: slice.brandExcludedMissing, brandExcludedNotInList: slice.brandExcludedNotInList, modelExcludedNotInList: slice.modelExcludedNotInList, excludeRemovedFromA: slice.excludeRemovedFromA },
      };
    ...
    if (body.wizardTask === "noveltiesFullExport") {
      ...
      if (tokenB.length < MIN_TOKEN_LEN) return NextResponse.json({ error: "Нужен ключ API для сайта B" }, { status: 400 });
      const rubricA = Number(body.rubricA);
      if (!Number.isFinite(rubricA) || rubricA < 1) return NextResponse.json({ error: "Некорректная rubricA" }, { status: 400 });
      ...
      return NextResponse.json({ resultKind: "noveltiesFullExport", products, siteBLabel, nameLocale } satisfies NoveltiesFullExportResult);
    }

    if (body.wizardTask === "noveltyIdsNoEanOnA") {
      const tokenA = resolveToken(body.tokenA, "A"), tokenB = resolveToken(body.tokenB, "B");
      if (tokenA.length < MIN_TOKEN_LEN || tokenB.length < MIN_TOKEN_LEN) return NextResponse.json({ error: "Нужны ключи API для сайтов A и B" }, { status: 400 });
      const rubricA = Number(body.rubricA);
      if (!Number.isFinite(rubricA) || rubricA < 1) return NextResponse.json({ error: "Некорректная rubricA" }, { status: 400 });
      const noveltyIds = parseNoveltyIdsFromBody(body.noveltyIdsB);
      if (!noveltyIds.length) return NextResponse.json({ error: "Пустой noveltyIdsB" }, { status: 400 });
      ...
      for (const p of mergedA.products) for (const e of collectEans(p)) if (e) eanOnA.add(e);
      ...
      for (const p of loaded) {
        ...
        if (eans.some((e) => e && eanOnA.has(e))) { removedForEanMatchOnA++; continue; }
        idsOut.push(p.id);
      }
      return NextResponse.json({
        resultKind: "noveltyIdsNoEanOnA", ids: idsOut, stats: { noveltyLoadedCount: loaded.length, removedForEanMatchOnA, remainingCount: idsOut.length }, siteALabel, siteBLabel,
    ...
    if (body.comparePhase === "noveltyIds") {
      ...
      const stage: NoveltyIdsStageResult = {
        resultKind: "noveltyIdsStage", siteALabel, siteBLabel, noveltyIds,
        stats: { countIdsRubricA: resA.ids.length, countIdsRubricB: resB.ids.length, idsOnBothSites, noveltyCount: noveltyIds.length },
        brandFilter: buildBrandFilterInfo(brandsRaw, brandMatch, resA.brandExcludedMissing, resA.brandExcludedNotInList, resB.brandExcludedMissing, resB.brandExcludedNotInList),
        modelFilter: buildModelFilterInfo(modelsRaw, modelMatch, resA.modelExcludedNotInList, resB.modelExcludedNotInList),
      ...
      if (singleDupsMode) {
        ...
        const single: SingleSiteDupsResult = {
          ...
          modelFilter: buildModelFilterInfo(
            modelsRaw,
            modelMatch,
            applied.modelExcludedNotInList,
            0
          ),
    ...
    if (singleDupsMode) {
      ...
      const single: SingleSiteDupsResult = {
        resultKind: "singleSiteDups", siteLabel: siteALabel, nameLocale, rubricId: rubricA, stats: { count: merged.products.length },
        brandFilter: buildBrandFilterInfo(brandsRaw, brandMatch, merged.brandExcludedMissing, merged.brandExcludedNotInList, 0, 0),
        ...
        eanGroups: dups.eanGroups, namePhotoPairs: dups.namePhotoPairs, brandVisualPairs: dups.brandVisualPairs, unlikelyPairs: dups.unlikelyPairs,
        unlikelySearch: buildUnlikelySearch(attrMatch),
      };
      return NextResponse.json(single);
    }
    ...
    if (siteBFetchMode === "noveltyIds") {
      if (!noveltyIdsB.length) return NextResponse.json({ error: "Пустой noveltyIdsB" }, { status: 400 });
      ...
      if (rubricBErrMain)
        return NextResponse.json({ error: rubricBErrMain }, { status: 400 });
    ...
    if (siteBFetchMode === "noveltyIds") {
      brandFilter = buildBrandFilterInfo(brandsRaw, brandMatch, mergedA.brandExcludedMissing, mergedA.brandExcludedNotInList, 0, 0);
      modelFilter = buildModelFilterInfo(modelsRaw, modelMatch, mergedA.modelExcludedNotInList, 0);
    } else if (mergedBForMeta) {
      brandFilter = buildBrandFilterInfo(brandsRaw, brandMatch, mergedA.brandExcludedMissing, mergedA.brandExcludedNotInList, mergedBForMeta.brandExcludedMissing, mergedBForMeta.brandExcludedNotInList);
      modelFilter = buildModelFilterInfo(modelsRaw, modelMatch, mergedA.modelExcludedNotInList, mergedBForMeta.modelExcludedNotInList);
    }
    const result: CompareResult = {
      ...cmp, brandFilter, modelFilter, excludeIdsA: buildExcludeIdsInfo(excludeIdsRaw, mergedA.excludeMeta), unlikelySearch: buildUnlikelySearch(attrMatch),
