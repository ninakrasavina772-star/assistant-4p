...
export async function POST(req: NextRequest) {
  ...
  try {
    if (body.mode === "idListIntraDups") {
      ...
      if (brandsRaw.length) {
        const br = filterFpProductsByBrands(products, brandsRaw, brandMatch);
        products = br.products;
        brandExcludedMissing = br.excludedMissingBrand;
        brandExcludedNotInList = br.excludedNotInList;
      }
      if (modelsRaw.length) {
        const mr = filterFpProductsByModels(products, modelsRaw, modelMatch);
        products = mr.products;
        modelExcludedNotInList = mr.excludedNotInList;
      }
      const dups = await findIntraSiteDuplicates(products, nameLocale, attrMatch);
      const single: SingleSiteDupsResult = {
        ...
        brandFilter: brandsRaw.length
          ...
          : undefined,
        modelFilter: modelsRaw.length
          ? buildModelFilterInfo(modelsRaw, modelMatch, modelExcludedNotInList, 0)
          : undefined,
