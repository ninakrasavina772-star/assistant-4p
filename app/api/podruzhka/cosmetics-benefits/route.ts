...
import { fetchCosmeticsBenefitsForRows, type CosmeticsBenefitsBatchIn } from "@/lib/podruzhkaCosmeticsAi";
import {
...
export async function POST(req: Request) {
  let body: CosmeticsBenefitsBatchIn;
  try {
    body = (await req.json()) as CosmeticsBenefitsBatchIn;
  } catch {
  ...
  try {
    const aiResults =
      valid.length > 0
        ? await fetchCosmeticsBenefitsForRows({
