...
export default function ComparePage() {
  ...
  const runCleanNovelties = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch("/api/compare", {
        ...
        signal: ac.signal,
        body: JSON.stringify({
          mode: "twoFeedsCleanNovelties",
          dataSource: "feeds",
          feedUrlA: feedUrlA.trim() || undefined,
          feedUrlB: feedUrlB.trim() || undefined,
          feedCsvTextA:
            !feedUrlA.trim() && feedCsvTextA.trim() ? feedCsvTextA : undefined,
          feedCsvTextB:
            !feedUrlB.trim() && feedCsvTextB.trim() ? feedCsvTextB : undefined,
          nameLocale,
          siteVariation,
      ...
      let json: (TwoFeedsCleanNoveltiesResult & { error?: string }) | { error?: string };
      try {
        json = (raw ? JSON.parse(raw) : {}) as typeof json;
      } catch {
        const oneLine = raw.replace(/\s+/g, " ").trim();
        setError(
          `Ответ сервера не JSON (HTTP ${res.status}). Фрагмент: ${oneLine.slice(0, 220) || "—"}`
        );
        return;
      }
      if (!res.ok || ("error" in json && json.error)) {
        setError(("error" in json && json.error) || `Ошибка ${res.status}`);
        return;
      }
      setCleanNoveltiesData(json as TwoFeedsCleanNoveltiesResult);
      window.setTimeout(() => {
        const el = document.getElementById("rep-clean-novelties");
      ...
      if (isAbort) {
        setError(
          userCancelledRef.current
            ? "Запрос отменён."
            : "Слишком долго: запрос остановлен (тайм‑аут 30 мин). Сузьте бренды или повторите позже."
        );
      } else {
  ...
  const downloadCleanNoveltiesXlsx = useCallback(async () => {
    if (!cleanNoveltiesData) return;
    setError(null);
    try {
      const { downloadCleanNoveltiesExcel } = await import(
        "@/lib/exportCleanNovelties"
      );
      await downloadCleanNoveltiesExcel(
        cleanNoveltiesData,
  ...
  const compareDisabledHint = !loading
    ? catalogSource === "feeds"
      ...
      : !brandsRequiredForTwoSite && compareMode === "twoSite"
        ...
        : !rubricAOk
          ? "Кнопка ожидает числовой id рубрики A: выберите в каскаде слева или введите id в поле под ним. Для каскада нужен ключ API 12+ симв.; без входа в сервис список рубрик с сервера недоступен — только ручной id."
  ...
  /** Основная задача страницы (четыре понятных сценария для контента) */
  ...
  const runIdListIntraDupSearch = useCallback(async () => {
    ...
    try {
      ...
      const raw = await res.text();
      let json: SingleSiteDupsResult & { error?: string };
      try {
        json = (raw ? JSON.parse(raw) : {}) as typeof json;
      } catch {
        const oneLine = raw.replace(/\s+/g, " ").trim();
      ...
      if (!("resultKind" in json) || json.resultKind !== "singleSiteDups") {
      ...
      if (isAbort) {
        setError(
          userCancelledRef.current
            ? "Запрос отменён."
            : "Таймаут или обрыв. Уменьшите число id и повторите."
        );
      } else {
        setError(e instanceof Error ? e.message : "Сеть");
      }
  ...
  return (
    <div className={appSubpageRoot}>
      ...
      >
        ...
        >
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <button
              type="button"
              onClick={() => applyCatalogMainTask("twoSite_cleanNovelties")}
              className={`rounded-xl border-2 p-4 text-left transition ${
      ...
      <section className={appSectionCard}>
        ...
        <div className="mb-6 rounded-xl border border-indigo-200 bg-indigo-50/50 px-4 py-3 space-y-2">
          ...
          <label className="flex items-start gap-2 text-sm text-slate-800 cursor-pointer">
            ...
            />
            <span>
              <strong className="text-slate-900">CSV или Excel</strong> — ссылка вида{" "}
              <code className="text-[11px] bg-white px-1 rounded border border-indigo-100">
                https://….4partners.io/my/feed/brand.csv
              </code>{" "}
              или файл <strong>.csv / .xlsx</strong> с тем же составом колонок (id товара, название, SKU/EAN и
              т.д. — подставляем автоматически). Не используйте ссылку и файл одновременно для одной стороны.
            </span>
          </label>
        </div>
        ...
        {catalogSource === "feeds" && (
          <div className="mb-6 rounded-xl border border-emerald-300/80 bg-emerald-50/40 px-4 py-4 space-y-4">
            <p className="text-sm text-slate-800 leading-relaxed">
              ...
              (колонки <strong className="whitespace-nowrap">Id товара</strong> или{" "}
              <strong className="whitespace-nowrap">Product Id</strong> и др.) сливаются в одну карточку
              (варианты: несколько EAN и картинок).
            </p>
          ...
          <div className="border-t border-amber-100/90 px-4 pb-4 pt-3 space-y-3">
            ...
            {status === "unauthenticated" && (
              <p className="rounded-lg border border-amber-200 bg-amber-50/80 px-3 py-2 text-xs text-amber-900">
                Каскад рубрик запрашивает сервер и требует{" "}
                <a href="/login" className="font-medium underline hover:text-amber-950">
                  входа
                </a>
                . Без входа остаётся только ввод id в полях под каскадом (из админки или API).
              </p>
            )}
            <div
            ...
            >
              ...
              {compareMode === "twoSite" && (
                <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm space-y-3">
                  ...
                  <RubricCascadeSelect
                    label={`Из каталога — клик по строке добавляет id (макс. ${MAX_RUBRICS_B})`}
                  ...
                  {rubricBParsedIds.length > 0 && (
                    <div>
                      <span className="text-xs text-slate-600">
                        Выбранные id ({rubricBParsedIds.length}/{MAX_RUBRICS_B}) — нажмите × чтобы убрать
                      </span>
                      <div className="mt-1.5 flex flex-wrap gap-1.5">
                        {rubricBParsedIds.map((rid) => (
                          <button
                            key={rid}
                            type="button"
                            onClick={() =>
                              setRubricsBText((prev) =>
                                removeRubricIdFromText(prev, rid)
                              )
                            }
                  ...
                  <label className="block">
                    <span className="text-xs font-medium text-slate-700">
                      Или введите id вручную — с новой строки или через запятую (до {MAX_RUBRICS_B})
                    </span>
                    <textarea
                    ...
                    <p className="mt-1 text-[11px] text-slate-500">
                      При таймаутах лучше несколько узких рубрик, чем одна большая родительская.
                    </p>
                  </label>
                </div>
              )}
        ...
        <details className="mb-4 rounded-xl border border-violet-200 bg-violet-50/35">
          <summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-slate-900">
            Пометить в файле дубли по одному EAN
          </summary>
          <div className="border-t border-violet-100 px-4 pb-4 pt-3 space-y-3">
            <p className="text-sm text-slate-800 font-medium leading-snug">
              Загрузите Excel или CSV — в ответ скачаете тот же файл с колонкой{" "}
              <span className="whitespace-nowrap">«{COL_DUP_RESULT}»</span>, где перечислены другие артикулы с тем же EAN.
              Интернет и сравнение витрин не нужны.
            </p>
            <p className="text-xs text-slate-500">
              В таблице должны быть заголовки <strong className="text-slate-700">Артикул</strong> и{" "}
              <strong className="text-slate-700">EAN</strong>. Для .xlsx берём лист «Новинки» или первый лист.
            </p>
            <details className="rounded-lg border border-violet-100/80 bg-white/60 px-3 py-2">
              <summary className="cursor-pointer text-xs font-medium text-violet-900">
                Технические детали (как в старом скрипте)
              </summary>
              <p className="mt-2 text-xs text-slate-600 leading-relaxed">
                Колонка <strong className="text-slate-800">«{COL_DUP_RESULT}»</strong> дописывается в конец или
                перезаписывается, если уже есть. Совпадающие артикулы в ячейке — через запятую с пробелом (в Apps Script у{" "}
                <code className="text-[11px]">join</code> был пустой разделитель — строки слипались).
              </p>
            </details>
            <div className="flex flex-wrap items-center gap-3 text-sm">
              <label className="text-xs font-medium text-slate-700 sr-only" htmlFor="ean-dup-file-input">
                Файл для проверки дублей по EAN
              </label>
              <input
                id="ean-dup-file-input"
                type="file"
                accept=".xlsx,.xls,.xlsm,.csv,.txt"
        ...
        <details className="mb-4 rounded-xl border border-teal-200 bg-teal-50/35">
          <summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-slate-900">
            Пометить в файле дубли по одинаковому названию
          </summary>
          <div className="border-t border-teal-100 px-4 pb-4 pt-3 space-y-3">
            <p className="text-sm text-slate-800 font-medium leading-snug">
              Загрузите файл — в ответ скачаете таблицу с колонкой{" "}
              <span className="whitespace-nowrap">«{COL_DUP_NAME_OR_PHOTO}»</span>: для каждой строки там будут{" "}
              <strong>ссылки из админки</strong> на другие строки с тем же названием. Картинки не сравниваются.
            </p>
            <p className="text-xs text-slate-500">
              Нужны колонки с названием товара и ссылкой в админку. Лист «Новинки» в .xlsx или первый лист — как в блоке
              выше.
            </p>
            <details className="rounded-lg border border-teal-100/80 bg-white/60 px-3 py-2">
              <summary className="cursor-pointer text-xs font-medium text-teal-900">
                Технические детали (как findDuplicatesByName)
              </summary>
              <p className="mt-2 text-xs text-slate-600 leading-relaxed">
                Строки с одинаковым <strong className="text-slate-800">«{COL_PRODUCT_NAME}»</strong> (после trim и нижнего
                регистра) — одна группа. В результат попадают уникальные{" "}
                <strong className="text-slate-800">«{COL_ADMIN_LINK}»</strong> с <em>других</em> строк группы; пустые ссылки
                пропускаются. Колонка <strong className="text-slate-800">«{COL_DUP_NAME_OR_PHOTO}»</strong> добавляется или
                перезаписывается. Несколько ссылок в ячейке — с новой строки (в Apps Script{" "}
                <code className="text-[11px]">join</code> был с пустым разделителем).
              </p>
            </details>
            <div className="flex flex-wrap items-center gap-3 text-sm">
              <label className="text-xs font-medium text-slate-700 sr-only" htmlFor="name-dup-file-input">
                Файл для проверки дублей по названию
              </label>
              <input
                id="name-dup-file-input"
                type="file"
                accept=".xlsx,.xls,.xlsm,.csv,.txt"
        ...
        <details className="mb-4 rounded-xl border border-sky-200 bg-sky-50/35">
          ...
          <div className="border-t border-sky-100 px-4 pb-4 pt-3 space-y-3">
            ...
            <p className="text-xs text-slate-600 bg-white/70 border border-sky-100 rounded-lg px-3 py-2">
              ...
              <strong className="text-slate-800">«Учитывать превью (vision)»</strong>, введите свой{" "}
            ...
            <textarea
              ...
              onChange={(e) => {
                setIdListDupIdsText(e.target.value);
                setError(null);
              }}
              rows={5}
            ...
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                disabled={idListDupLoading || idListDupParsedIds.length === 0}
                onClick={() => void runIdListIntraDupSearch()}
                className="rounded-lg border border-sky-500 bg-sky-600 px-3 py-2 text-xs font-semibold text-white hover:bg-sky-700 disabled:opacity-50 disabled:pointer-events-none"
              >
                {idListDupLoading ? "Идёт запрос…" : "Найти дубли по списку"}
              </button>
              <button
                type="button"
                disabled={idListDupLoading}
                onClick={() => {
                  setIdListDupIdsText("");
                  setError(null);
                }}
            ...
            {idListDupLoading && (
              <p className="text-xs text-sky-900">
                Загрузка карточек и расчёт дублей может занять время при большом списке (много пар и превью).
              </p>
            )}
          </div>
        </details>
          ...
          <>
            <details className="mb-6 rounded-2xl border border-slate-200 bg-slate-50/50 shadow-sm">
              <summary className="cursor-pointer list-none px-4 py-3 text-sm font-semibold text-slate-800 [&::-webkit-details-marker]:hidden">
                Старый сценарий сравнения (кнопка внизу и порядок отчётов)
              </summary>
                  ...
                  {data.stats.idList.missingInApi > 0 ? (
                    <>
                      ...
                      <strong className="tabular-nums text-amber-800">
                        {data.stats.idList.missingInApi}
                      </strong>
                    </>
                  ) : null}
                </>
      ...
      {cleanNoveltiesData && (
        <section
          id="rep-clean-novelties"
          className="mt-6 rounded-2xl border border-emerald-300/80 bg-emerald-50/40 p-4 sm:p-6 scroll-mt-24"
        >
          <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
            <h2 className="text-base font-bold text-emerald-950">
              Чистый фид {cleanNoveltiesData.siteBLabel} (без дублей на {cleanNoveltiesData.siteALabel})
            </h2>
            <button
              type="button"
