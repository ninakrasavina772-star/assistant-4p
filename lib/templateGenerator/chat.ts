...
export function welcomeMessage(): ChatMessage {
  return {
    ...
    at: Date.now(),
    content:
      "Привет! Я помогу заполнить шаблон Ozon. Загрузите Excel-шаблон и при необходимости CSV (файлом или ссылкой). Когда файлы будут готовы — напишите, что заполнять: например «описание и ноты, описание продающее, факты с официального сайта». Я запомню всё до конца сессии."
  };
}
