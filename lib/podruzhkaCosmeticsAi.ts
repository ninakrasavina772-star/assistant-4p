...
2. benefits — ровно 3 объекта {title, desc}:
   ...
   - desc: 2–6 слов по-русски, с маленькой буквы, без точки; поясняет выгоду
...
6. ЗАПРЕЩЕНО: медицинские обещания, «лечит», «100%», выдуманные SPF/состав, английский в title.
...
function resultFromParsed(row: PodruzhkaFeedRow, parsed: AiJson): PodruzhkaAiResult {
  ...
  if (!modelName || benefits.length < 3 || benefitErr) {
    return {
      row: row.row,
      ok: false,
      model: modelName,
